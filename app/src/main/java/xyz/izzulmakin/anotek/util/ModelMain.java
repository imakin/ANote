...
public class ModelMain {
     ...
     */
    public static void initDatabase(Context mref) {
        dbmain = mref.openOrCreateDatabase("anoteappdb.sqlite", mref.MODE_PRIVATE, null);
        ModelMain.mref = mref;
     ...
     * @see ModelMain.provisioningApply
    ...
    public static void provisioningApply() {
        Cursor dbc_provisioning = getdb().rawQuery(
        ...
        while (dbc_provisioning.moveToNext()) {
            ...
            if (!created && should_be) {
                ...
                getdb().execSQL("CREATE TABLE IF NOT EXISTS " + tablename + "("+fields+")");
                if (!unique_index.equals("")) {
                    getdb().execSQL("CREATE UNIQUE INDEX IF NOT EXISTS "+unique_index+" ON "+tablename+" ("+unique_index+")");
                }
            }
